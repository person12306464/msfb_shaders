...
float4 PS_ComputeGI(float4 position : SV_Position, float2 coord : TEXCOORD0) : SV_Target
{
    ...
    float3 screen_position = float3(coord, depth);
    float3 ray_dir = normal; // normal for ray direction
    
    // ray increment
    float RayInc = RAY_INC * dims.x;

    // initialize variables for reflection accumulation
    ...
    for (int i = 0; i < SAMPLES; i++)
    {
        ...
        float3 rayPos = screen_position;

        for(int j = 0; j < RAY_LEN; j++)
        {
            rayPos += sampleRay * RayInc;
            float2 sampleCoord = rayPos.xy;
            ...
            // adjust color accumulation based on depth difference
    ...
    float specular = pow(max(dot(viewDir, reflectDir), 0.0), SPEC_SIZE);
    DebugColor += SPEC_STR * specular;
